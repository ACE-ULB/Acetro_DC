...
class Carte_ACE(commands.Cog, Events, Logs, name=TEMPLATES_NAME):
	...
	async def on_raw_reaction_add(self, payload):
		if not payload.member.bot:
			log_type = await self.switch_reaction(payload)
			await self.switch_logs(payload, log_type, 'success')
	...
	async def initial_config(self, ctx):
		...
		await self.switch_logs(ctx, 'initial_config', 'success')
		await self.init_setup_pwd(ctx)

	async def init_setup_pwd(self, ctx):
		msg = await ctx.send(txt_cmd_emojis['pause']+" "+wel_create_pwd)
		await add_reaction(converter_text_to_emojis(txt_ctrl_emojis['valid'])[0])